...
async function loadPartnerFeed(){
  try{
    ...
    html+=feed.map(f=>{
      ...
      const loc=window.clientLocale;
      const localized=tr&&loc&&tr[loc];
      const fTitle=localized?localized.title:f.title;
      const fSummary=localized?localized.insight:f.summary;
      return`<div style="background:var(--card);border:1px solid var(--border);border-radius:8px;padding:10px 12px;margin-bottom:8px;">
        ...
        <div style="font-size:.72rem;color:var(--text);font-weight:600;margin-bottom:3px;">${fTitle||''}</div>
        <div style="font-size:.65rem;color:var(--muted);">${(fSummary||'').substring(0,80)}${(fSummary||'').length>80?'...':''}</div>
      </div>`;
    }).join('');
