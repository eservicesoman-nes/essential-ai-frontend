...
function addCustomCredField(clientId){
  ...
  if(!grid)return;
  const label=prompt(t('miscPanelsUi.fieldNamePrompt'));
  if(!label||!label.trim())return;
  const key=label.trim().toLowerCase().replace(/\s+/g,'_').replace(/[^a-z0-9_]/g,'');
  const div=document.createElement('div');
  div.className='cred-field';
  div.innerHTML=`<div class="cf-lbl">${label.trim()}</div><input class="cf-input" type="text" placeholder="${t('miscPanelsUi.value')}" data-field="${key}">`;
